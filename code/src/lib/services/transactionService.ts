...
export function createBlankForm(): TransactionForm {
  return {
    id: null,
    type: 'expense',
    category: getCategoryChoicesForType('expense')[0],
    amount: '',
...
export function applyTransactionMutation({
  ...
  const submitted: Transaction = {
    id: form.id ?? makeId(),
    type: form.type,
    category: normalizeImportedCategory(form.type, validation.normalized.category),
    amount: Number(Number(validation.normalized.amount).toFixed(2)),
    date: form.date,
...
export function startEditingTransaction(item: Transaction): {
  formMode: 'edit';
  form: TransactionForm;
} {
  return {
